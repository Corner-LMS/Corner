...
export default function ComposeMessageScreen() {
    ...
    const preSelectedUserId = params.userId as string;
    const preSelectedUserName = params.userName as string;
    ...
    const [selectedUser, setSelectedUser] = useState<User | null>(
        preSelectedUserId && preSelectedUserName
            ? { id: preSelectedUserId, name: preSelectedUserName, email: '', role: '' }
            : null
    );
    ...
    const loadUsers = async () => {
        try {
            ...
            if (currentUserRole === 'teacher') {
                // Teachers can message students from their courses
                const coursesQuery = await firestore()
                    .collection('courses')
                    .where('teacherId', '==', user.uid)
                    .get();

                const courseIds = coursesQuery.docs.map(doc => doc.id);

                if (courseIds.length > 0) {
                    usersQuery = await firestore()
                        .collection('users')
                        .where('role', '==', 'student')
                        .where('courseIds', 'array-contains-any', courseIds)
                        .get();
                } else {
                    setUsers([]);
                    return;
                ...
                // Students can message teachers from their courses
                const courseIds = currentUserData?.courseIds || [];
                ...
                if (courseIds.length > 0) {
                    ...
                    const teacherIds = coursesQuery.docs.map(doc => doc.data().teacherId);
